...
import { Repository, TeamMembers, ProjectData, EnvVar, DockerConfig, TeamRole, TeamMember } from '@/types';
...
export const useProjectData = () => {
  ...
  const [repositories, setRepositories] = useState<Record<string, Repository>>(initialRepositories);
  const [teamMembers, setTeamMembers] = useState<TeamMembers>(initialTeamMembers);

  const updateProjectName = useCallback((name: string) => {
    setProjectName(name);
  }, []);

  const addRepository = useCallback((name: string, url: string) => {
  ...
  const updateRepositoryBranch = useCallback((repoName: string, branch: string) => {
    setRepositories(prev => ({
      ...prev,
      [repoName]: {
        ...prev[repoName],
  ...
  const addEnvVar = useCallback((repoName: string, envVar: EnvVar) => {
    setRepositories(prev => {
      ...
      const existingIndex = repo.envVars.findIndex(v => v.name === envVar.name);
  ...
  const parseBulkEnvVars = useCallback((repoName: string, envText: string) => {
  ...
  const addDockerConfig = useCallback((repoName: string, config: Omit<DockerConfig, 'id'>) => {
    setRepositories(prev => {
      const repo = prev[repoName];
      if (!repo) return prev;
  ...
  const updateDockerConfig = useCallback((repoName: string, index: number, config: DockerConfig) => {
    setRepositories(prev => {
      const repo = prev[repoName];
      if (!repo) return prev;
